...
- **命令**：``MONITORING_TRIGGER_REQUIRES_AT_BOT=1`` 时须 @ **本**机器人再发 ``/mo``；若 HTTP 完全不带 ``mentions``，默认 ``MONITORING_MO_ALLOW_FEISHU_AT_PLACEHOLDER=1`` 才用正文 ``@_user_N`` 兜底。**若 mentions 里已是别的 bot，不会误触发**（多 bot 同群）。未配 ``LARK_BOT_OPEN_ID`` 时会尝试 ``GET bot/v3/info``。``/m`` / ``/c`` 静音；机器人回复为英文。
...
def _text_should_run_monitoring(raw_text: str, clean: str, mentions: Any) -> bool:
    ...
    ``MONITORING_MO_ALLOW_FEISHU_AT_PLACEHOLDER`` only applies when ``mentions`` is **completely empty**
    (HTTP omitted meta) yet ``@_user_N`` remains in the body.
    """
