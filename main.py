...
- **命令**：``MONITORING_TRIGGER_REQUIRES_AT_BOT=1`` 时须 @ **本**机器人再发 ``/mo``；默认 ``MONITORING_MO_ALLOW_FEISHU_AT_PLACEHOLDER=1`` 时可用正文 ``@_user_N`` 兜底（含 Feishu 只下发弱 ``mentions``、无 ``ou_``/``cli_`` 元数据的情形）。若 mentions 里含 **他人** 的 ``ou_``/``cli_`` 等强 id，其它 bot 不会误触发。未配 ``LARK_BOT_OPEN_ID`` 时会尝试 ``GET bot/v3/info``。``/m`` / ``/c`` 静音；机器人回复为英文。
