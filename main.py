...
- **命令**：``MONITORING_TRIGGER_REQUIRES_AT_BOT=1`` 时须 @ **本**机器人再发 ``/mo``；``MONITORING_MO_WEAK_NONEMPTY_MENTIONS_ALLOW=1``（默认）允许「非空弱 mentions + 正文 ``@_user_N``」触发（Game 需要）。**同群的 Platform bot 须置 ``MONITORING_MO_WEAK_NONEMPTY_MENTIONS_ALLOW=0``**，否则会跟你一起回 ``/mo``。``MONITORING_MO_ALLOW_FEISHU_AT_PLACEHOLDER`` 仍用于 **mentions 完全为空** 的兜底。未配 ``LARK_BOT_OPEN_ID`` 时会尝试 ``GET bot/v3/info``。``/m`` / ``/c`` 静音；机器人回复为英文。
...
def _text_should_run_monitoring(raw_text: str, clean: str, mentions: Any) -> bool:
    ...
    Turn **off** on bots that share a Feishu group with another monitoring bot (otherwise both may reply).
    """
