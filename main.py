...
def _text_should_run_monitoring(
    ...
    *,
    ...
    ``MONITORING_CANONICAL_BOT_OPEN_ID`` / …: merged set. Intersection → trigger. Disjoint → **hard skip**
    peer-only only when **body** ``<at>`` ids (not mentions-alone) are subset of ``MONITORING_PEER_BOT_OPEN_IDS``;
    mentions-only peer ids with ``@_user_N`` fall through to weak nonempty on Game.
    """
