...
_CFG: Dict[str, Any] = {
    ...
    "MONITORING_CANONICAL_BOT_OPEN_IDS": "ou_848fc4640b48b9845cbc5b0cfa2f1af1",
    # Platform 机器人可能出现的 open_id（逗号/空格）。弱 mentions 兜底时若 <at> 只指向 peer 则不回
    "MONITORING_PEER_BOT_OPEN_IDS": "ou_0bfd185231d6beb669425fdf8f13e9df ou_ee1af664e18d9c2d25e0ab6fded66388",
    "LARK_ENCRYPT_KEY": "",
    "LARK_BOT_OPEN_ID": "",
...
def _text_should_run_monitoring(
    ...
    *,
    ...
    When ``MONITORING_TRIGGER_REQUIRES_AT_BOT`` is true, an explicit trigger (e.g. ``/mo``) runs if
    ``mentions`` includes this bot. If ``mentions`` is **non-empty** but lists another bot/user only,
    we **do not** fall back to ``@_user_N`` text (avoids wrong bot replying in multi-bot groups).

    ``MONITORING_MO_ALLOW_FEISHU_AT_PLACEHOLDER``: when ``mentions`` is **empty**, body ``@_user_N`` still
    needs this flag. When ``mentions`` is **non-empty** but only **weak** (no conflicting ``ou_``/``cli_``
    for someone else), ``@_user_N`` in the body is enough — avoids systemd accidentally setting the flag
    to ``0`` while Feishu keeps sending placeholder-style payloads.

    Strong ``ou_``/``cli_`` ids for **another** entity still block ``/mo``.

    ``MONITORING_MO_WEAK_NONEMPTY_MENTIONS_ALLOW``: enable the non-empty weak ``mentions`` + ``@_user_N`` path.

    ``MONITORING_PEER_BOT_OPEN_IDS``: when ``content`` contains ``<at user_id=\"ou_…\">`` for a peer bot only,
    skip weak-nonempty ``/mo`` here (set to the **other** bot's ``open_id`` on each app).

    If ``mentions[]`` is weak but ``content`` JSON still contains ``<at …>`` with **this** bot's ``open_id``
    (requires ``LARK_BOT_OPEN_ID`` or working ``bot/v3/info``), ``/mo`` triggers — fixes Platform with
    ``MONITORING_MO_WEAK_NONEMPTY_MENTIONS_ALLOW=0``.

    ``MONITORING_CANONICAL_BOT_OPEN_ID`` / ``MONITORING_CANONICAL_BOT_OPEN_IDS`` / runtime ``bot/v3/info``: merged into
    one set; when non-empty and we parse explicit ``ou_``/``cli_`` targets, trigger only if that set **intersects**
    explicit ids (wrong-process webhook guard without brittle single-string mismatch).
    """
