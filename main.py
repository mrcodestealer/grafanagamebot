...
- **命令**：``MONITORING_TRIGGER_REQUIRES_AT_BOT=1`` 时须 @ 本机器人再发 ``/mo``；HTTP 常不带 ``mentions``，默认 ``MONITORING_MO_ALLOW_FEISHU_AT_PLACEHOLDER=1`` 允许正文里保留 ``@_user_N`` 占位时也触发。未配 ``LARK_BOT_OPEN_ID`` 时会尝试 ``GET bot/v3/info``。``/m`` / ``/c`` 静音；机器人回复为英文。
