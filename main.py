...
默认 ``MONITORING_MESSAGE_CARD_ENABLE=1``：监控回复为交互卡片；``MONITORING_MESSAGE_CARD_BUTTON_ENABLE=1`` 时可用 **Resend screenshot**（callback）。若改 ``0`` 则改为纯文字 + 独立图，**无**该按钮。
"""
...
_CFG: Dict[str, Any] = {
    ...
    # Platform 机器人可能出现的 open_id（逗号/空格）；须列全以便 peer-only skip（飞书同一 bot 可能多条 ou_）
...
def _text_should_run_monitoring(
    ...
    *,
    ...
    Intersection → trigger. Disjoint → **skip only** if explicit ids are **all** in ``MONITORING_PEER_BOT_OPEN_IDS``;
    else **fall through** for Feishu ``open_id`` variants not listed in canon.
    """
